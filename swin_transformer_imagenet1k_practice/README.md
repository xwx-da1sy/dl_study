# ImageNet-1K 四阶段 Swin Transformer 实践

本目录从 CIFAR-100 实践项目独立复制而来，保留原有文件结构、Attention Pooling、训练、搜索和可视化流程，只把数据集替换为 ImageNet-1K，并把网络扩展为四个 Stage。

学习顺序：

1. `01_pretrained_swin_t_inference_and_shape_trace.py`：运行 torchvision 预训练 Swin-T，并把真实输出与四阶段理论 shape 对齐。
2. `swin/config.py`：集中保存路径、ImageNet-1K数据、四阶段网络结构和训练配置。
3. `swin/data.py`：读取 ImageNet-1K，从官方train按类别均衡划分内部验证集，并提供Mixup/CutMix。
...
7. `swin/model.py`：组装四阶段自定义 Swin，并继续使用轻量Attention Pooling输出1000类logits。
...
10. `search_hyperparameters.py`：固定内部训练/验证划分，搜索学习率和Weight Decay；ImageNet官方val不参与选择。
11. `train_custom_swin.py`：使用搜索出的最佳参数完整训练300轮，并保存验证集表现最好的模型。
12. `swin/evaluation.py`：统计 Top-1/Top-5、逐类别准确率和混淆矩阵，并绘制训练与预测结果。
13. `evaluate_custom_swin.py`：确定最终配置后，只在ImageNet官方val上评估一次并生成结果文件。
...
└── val/
    ├── n01440764/
    ├── n01443537/
    └── ...与train完全相同的1000个类别目录
```

默认从官方train的每个类别固定划出50张图片，组成50,000张内部验证集；官方val只供最终评估。
...
4. 最终模型确定后，再运行 `python evaluate_custom_swin.py` 使用ImageNet官方val。
