"""224×224 ViT 的集中配置（CIFAR-10 放大练手版）。

参数和 vit/config.py（32×32 当前版）完全对齐，只改三处：
  1. IMAGE_SIZE 32 -> 224（CIFAR-10 Resize 放大）
  2. PATCH_SIZE 4 -> 16（224/16=14 → 196 patches，标准 ViT 配置）
  3. 路径用 checkpoints_224 / results_224，避免和原 32×32 版冲突

注意：放大不增加真实信息（32×32 是天花板），本工程目的是练手 224 ViT 架构流程。
"""
...
# 用独立目录 checkpoints_224，避免和原 32×32 版的 checkpoints 冲突。
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints_224"
BEST_MODEL_PATH = CHECKPOINT_DIR / "vit224_best.pt"

# 结果保存到独立目录 results_224，避免和原 32×32 版的 results 冲突。
RESULTS_DIR = PROJECT_ROOT / "results_224"
...
# 224×224 比 32×32 显存占用大很多；显存不足时减小 batch_size。
BATCH_SIZE = 128
...
# 224×224 输入与 Patch Embedding 配置。
IMAGE_SIZE = 224
PATCH_SIZE = 16  # 224 / 16 = 14 → 14 x 14 = 196 个 patch
IN_CHANNELS = 3
EMBED_DIM = 256
...
# Early Stopping：验证 loss 连续 50 个 epoch 没有实质改善时停止训练。
EARLY_STOPPING_PATIENCE = 50
EARLY_STOPPING_MIN_DELTA = 1e-5

# 余弦退火最终将学习率从 4e-4 平滑降低到 1e-6。
