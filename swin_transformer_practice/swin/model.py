"""三阶段自定义 Swin、轻量 Attention Pooling 与分类输出。"""
...
class ClassificationHead(nn.Module):
    """把整图特征从384维映射为 CIFAR-100 的100个 logits。"""
...
class CustomSwin(nn.Module):
    """针对 32 x 32 CIFAR-100 图片设计的三阶段 Swin 分类网络。"""
    ...
    def __init__(
        ...
        if not (
            ...
            raise ValueError("当前网络要求三个 Stage 的配置长度都为3")
        if stage_dims[0] != embed_dim:
            raise ValueError("Stage 1 通道数必须等于 Patch Embedding 维度")
        if stage_dims[1] != 2 * stage_dims[0] or stage_dims[2] != 2 * stage_dims[1]:
    ...
    def forward_features(self, images):
        # B x 3 x 32 x 32 -> B x 16 x 16 x 96。
        x = self.patch_embedding(images)
        x = self.stage1(x)

        # B x 16 x 16 x 96 -> B x 8 x 8 x 192。
        x = self.patch_merging1(x)
        x = self.stage2(x)

        # B x 8 x 8 x 192 -> B x 4 x 4 x 384。
        x = self.patch_merging2(x)
        x = self.stage3(x)
        x = self.final_layer_norm(x)

        # B x 4 x 4 x 384 -> B x 16 x 384。
