...
def parse_args():
    parser = argparse.ArgumentParser(
        description="评估并可视化 CIFAR-100 自定义 Swin"
    )
...
def main():
    ...
    _, _, test_loader, class_names = create_cifar100_dataloaders(
    ...
    # 测试 loss 使用普通交叉熵，Label Smoothing 只用于训练正则化。
    ...
    print(
        "测试集 Top-1 / Top-5："
        f"{summary['test_top1_accuracy'] * 100:.2f}% / "
        f"{summary['test_top5_accuracy'] * 100:.2f}%"
    )
